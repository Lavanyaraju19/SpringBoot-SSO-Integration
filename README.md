# SpringBoot-SSO-Integration
A demo project integrating Single Sign-On (SSO) in a Spring Boot application using Spring Security. It shows how to configure secure authentication with external identity providers like Google or custom SSO solutions using OpenID Connect (OIDC) and JWT for seamless user login.

Key Features
Single Sign-On (SSO): Allows users to authenticate via external identity providers (e.g., Google, Facebook, or corporate identity providers).

Spring Security: Configures authentication and authorization in the Spring Boot application.

OpenID Connect (OIDC): Implements the OIDC protocol for seamless authentication.

JWT (JSON Web Tokens): Used for secure session management and token-based authentication.

Easily configurable to integrate with various identity providers (IDPs).
...
An external Identity Provider (e.g., Google, Okta) with OIDC support
